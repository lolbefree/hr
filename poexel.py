...
name_file = 'Стаж роботи працівників на АВТОСОЮЗ.xlsx'
...
for i in all_rows:
    if "Дата прийняття" not in i and i is not None:
        try:
...
print("\n", "До року кількість співробітників: ", int(len(less_year)/3), "\n",
      "Від року до трьох кількість співробітників: ", int(len(from_one_year_to_three)/3), "\n",
      "Від трьох до десяти кількість співробітників: ", int(len(from_three_year_to_ten)/3), "\n",
      "Більше 10 років кількість співробітників:", int(len(after_ten_years)/3))
time.sleep(6)
del _month_
del _years_
wb.save(name_file)
